    ...
    <form action="/update/{{ $anime->id }}/{{ $review->id }}" method="POST">
        ...
        @method('PUT')
            <div class="mb-4">
                ...
                @endif
                <input type="number" name="review[star]" class="block w-full sm:w-2/3 bg-gray-200 py-2 px-3 text-gray-700 border border-gray-200 rounded focus:outline-none focus:bg-white" placeholder="星の数" value="{{ $review->star }}"/>
            </div>
            <div class="mb-4">
                ...
                @endif
                <input type="text" size="30" name="review[comment_title]" class="block w-full sm:w-2/3 bg-gray-200 py-2 px-3 text-gray-700 border border-gray-200 rounded focus:outline-none focus:bg-white" placeholder="口コミタイトル" value="{{ $review->comment_title }}"/>
            </div>
            <div class="mb-4">
                ...
                @endif
                <textarea rows="10" cols="40" name="review[comment]"  class="block w-full sm:w-2/3 bg-gray-200 py-2 px-3 text-gray-700 border border-gray-200 rounded focus:outline-none focus:bg-white" placeholder="口コミを投稿する">{{ $review->comment }}</textarea>
            </div>
